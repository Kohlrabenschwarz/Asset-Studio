...
namespace AssetStudio.GUI
{
    partial class MainForm : Form
    {
        ...
        private void InitializeReplaceMenu()
        {
            var replaceMenu = new ToolStripMenuItem("Replace");
            replaceMenu.DropDownItems.Add("Replace selected asset...", null, (_, _) => ReplaceSelectedAsset(false));
            replaceMenu.DropDownItems.Add("Replace raw serialized bytes...", null, (_, _) => ReplaceSelectedAsset(true));
            menuStrip1.Items.Insert(menuStrip1.Items.IndexOf(exportToolStripMenuItem), replaceMenu);
        }

        private async void ReplaceSelectedAsset(bool raw)
        {
            if (assetListView.SelectedIndices.Count != 1)
            {
                MessageBox.Show(this, "Select exactly one asset in the asset list.", "Replace", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            var selected = (AssetItem)assetListView.Items[assetListView.SelectedIndices[0]];
            if (!raw && !AssetReplacementService.CanImport(selected.Type))
            {
                MessageBox.Show(this, $"Automatic replacement is not available for {selected.Type}. Use the raw option if you have serialized object bytes.", "Replace", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            using var inputDialog = new OpenFileDialog
            {
                Title = $"Choose replacement for {selected.Text} ({selected.Type})",
                Filter = raw ? "Raw serialized assets|*.dat;*.bytes|All files|*.*" : AssetReplacementService.InputFilter(selected.Type),
                FilterIndex = 1,
            ...
            string sourcePath = selected.SourceFile.originalPath ?? selected.SourceFile.fullName;
            using var outputDialog = new SaveFileDialog
            {
                Title = "Save modified Unity file",
                InitialDirectory = Path.GetDirectoryName(sourcePath),
                FileName = Path.GetFileName(sourcePath) + ".modified",
                Filter = "All files|*.*",
            ...
            {
                MessageBox.Show(this, "The output file already exists. Choose a new file name.", "Replace", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            try
            {
                ...
                StatusStripUpdate($"Replacing {selected.Text}...");
                await Task.Run(() => AssetReplacementService.Replace(selected, inputDialog.FileName, outputDialog.FileName, raw));
                StatusStripUpdate($"Saved {outputDialog.FileName}");
                MessageBox.Show(this, $"Modified file saved:\n{outputDialog.FileName}", "Replace", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            catch (Exception ex)
            {
                Logger.Error("Unable to replace asset", ex);
