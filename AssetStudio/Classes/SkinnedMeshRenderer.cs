...
namespace AssetStudio
{
    public sealed class SkinnedMeshRenderer : Renderer
    {
        ...
        public SkinnedMeshRenderer(ObjectReader reader) : base(reader)
        {
            ...
            if (nodes?.Any(x => x.m_Level == 1 && x.m_Name == "m_RootBone") ?? reader.Game.Type.IsGIGroup())
            {
                m_RootBone = new PPtr<Transform>(reader);
            }
            if (nodes?.Any(x => x.m_Level == 1 && x.m_Name == "m_AABB") ?? reader.Game.Type.IsGIGroup())
            {
                m_AABB = new AABB(reader);
            }
            if (nodes?.Any(x => x.m_Level == 1 && x.m_Name == "m_DirtyAABB") ?? reader.Game.Type.IsGIGroup())
            {
