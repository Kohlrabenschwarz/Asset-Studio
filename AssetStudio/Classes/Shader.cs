...
namespace AssetStudio
{
    ...
    public class SerializedPass
    {
        ...
        public SerializedPass(ObjectReader reader)
        {
            ...
            var passNodes = reader.serializedType?.m_Type?.m_Nodes;
            if (passNodes?.Count == 0) passNodes = null;
            if (version[0] > 2020 || (version[0] == 2020 && version[1] >= 2)) //2020.2 and up
            {
                if (passNodes?.Any(x => x.m_Name == "m_EditorDataHash") ?? version[0] < 6000)
                {
                ...
                if (passNodes?.Any(x => x.m_Name == "m_Platforms") ?? version[0] < 6000)
                {
