        ...
        protected Renderer(ObjectReader reader) : base(reader)
        {
            ...
            else //5.0 and up
            {
                if (version[0] > 5 || (version[0] == 5 && version[1] >= 4)) //5.4 and up
                {
                    ...
                    var rendererNodes = reader.serializedType?.m_Type?.m_Nodes;
                    if (rendererNodes?.Count == 0) rendererNodes = null;
                    if (rendererNodes?.Any(x => x.m_Level == 1 && x.m_Name == "m_RayTracingAccelStructBuildFlagsOverride") ?? version[0] >= 6000)
                    {
                        var m_RayTracingAccelStructBuildFlagsOverride = reader.ReadByte();
                    }
                    if (rendererNodes?.Any(x => x.m_Level == 1 && x.m_Name == "m_RayTracingAccelStructBuildFlags") ?? version[0] >= 6000)
                    {
                        var m_RayTracingAccelStructBuildFlags = reader.ReadByte();
                    }
                    if (rendererNodes?.Any(x => x.m_Level == 1 && x.m_Name == "m_SmallMeshCulling") ?? version[0] >= 6000)
                    {
                    ...
                    reader.AlignStream();
                    if (rendererNodes?.Any(x => x.m_Level == 1 && x.m_Name == "m_ForceMeshLod") ?? version[0] >= 6000)
                    {
                        var m_ForceMeshLod = reader.ReadInt16();
                        reader.AlignStream();
                    }
                    if (rendererNodes?.Any(x => x.m_Level == 1 && x.m_Name == "m_MeshLodSelectionBias") ?? version[0] >= 6000)
                    {
